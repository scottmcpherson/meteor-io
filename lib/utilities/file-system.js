...
class FileSystem extends Logger {
  ...
  move(src, dest) {
    fs.readdirSync(src).forEach((path) => {
      if (path !== 'app') {
        let oldDir = this.join(src, path);
        let newDir = this.join(dest, path);
  ...
  copy(src, dest) {
    ...
    if (self.confirmFileOverwriteIfExists(dest)) {
  ...
  writeFile(file) {
    var self = this;
    if (self.confirmFileOverwriteIfExists(file)) {
  ...
  confirmFileOverwriteIfExists(file) {
    ...
    if (self.isFile(file)) {
      ...
      let isConfirmed = confirmWithUser('Do you want to overwrite it?');
      return isConfirmed;
    }
    return true;
  }
  confirmWithUser(question) {
    let isConfirmed = readlineSync.question(`${question} [y/n] : `, {
      trueValue: ['yes', 'y'],
      falseValue: ['no', 'n']
    });
    return isConfirmed;
