...
class FileSystem extends Logger {
  ...
  get configFile () {
    try {
      var configPath = this.join(this.ioPath, '.io', 'config.json');
  ...
  get _adminConfigFilePath () {
    return path.join(this._adminConfigPath, 'config.json');
  }
  ...
  get _adminCofig () {
    try {
      var json = fs.readJsonSync(this._adminConfigFilePath);
      return json;
    } catch (e) {
      this.logError(e);
      return false;
  ...
  get ioPath () {
    return this._adminCofig.ioPath;
  }
