...
class Utils extends FileSystem {
  ...
  camelizeFileName(fileName) {
    this.logWarning('camelizeFileName is depreciated. Use camelize() instead');
  ...
  convertConstantToVariable(constant) {
    ...
    return this.camelize(underscoreToDash);
  }
  converFileToClassName(fileName) {
    this.logWarning('converFileToClassName is depreciated. Use classify() instead');
    return this.classify(fileName);
  }
