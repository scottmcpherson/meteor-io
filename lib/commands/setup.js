  ...
  run (args, argv) {
    ...
    rl.question("Directory: ", function(ionDest) {
      ...
      if (self.isDir(ionDest)) {
        try {
          let usersIonPath = path.join(ionDest, 'ion');

          // Create user's ion directory
          self.mkdir(usersIonPath);
          ...
          fs.writeJsonSync(adminConfigPath, { usersIonPath: usersIonPath });

          let templatesPath = path.join(usersIonPath, 'templates');
          let generatorsPath = path.join(usersIonPath, 'generators');
