...
// TODO: Need to move the --generator and --command flag
// generation code into their own seperate commands or generators
class Create extends Command {
  constructor() {
    super();
    this.name = 'create';
    this.description = 'io create <project-name>, io create {generator-name, command-name} {--generator, --command}';
  }
  run(args, argv) {
    var self = this;
    ...
    } else if (argv.command && args.length > 1) {
      let commandName = args[1];
      let commandFileName = commandName + '.js';
      let className = this.converFileToClassName(commandName);
      let userIoCommandPath = path.join(self.ioPath, 'commands', commandFileName);
